...
impl SectorMesh {
    pub fn from_sector_view(sector_view: &SectorView, render_context: &RenderContext) -> Self {
        let sector_mesh = Self::lysenko_optimization(sector_view, render_context);

        sector_mesh
    }

    fn lysenko_optimization(sector_view: &SectorView, render_context: &RenderContext) -> Self {
        let mask_vec = Self::generate_mask_vec(sector_view, render_context);
                    ...
                    if let Some(block) = Sector::get_block(cell_index, &sector_view.block_vec) {
                        ...
                        if DirectionSet::has(&Direction::North, &block.block_state.exposure_set) {
                            ...
                            let sector_face = SectorQuad {
                                ...
                                texture_layer_index,
                                uv_array: RenderCatalog::BLOCK_UV_ARRAY.clone(),
                            };
    ...
    pub fn emit_triangles(
        ...
        let normal: [f32; 3] = *Direction::to_vec3(&sector_quad.direction).as_array();
        ...
        let (uv0, uv1, uv2, uv3) = match sector_quad.direction {
        ...
        vertex_vec.push(SectorVertex {
            ...
            normal,
            uv: uv0,
            layer_index: sector_quad.texture_layer_index.into(),
        });
        ...
        let use_canonical = match sector_quad.direction {
