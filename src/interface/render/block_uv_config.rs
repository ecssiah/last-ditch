...
pub struct BlockUVConfig {
    pub kind: block::Kind,
    pub tile_position: HashMap<String, (u32, u32)>,
}
...
impl TryFrom<BlockUVConfig> for render::BlockUV {
    ...
    fn try_from(block_uv_config: BlockUVConfig) -> Result<Self, Self::Error> {
        let mut tile_position = HashMap::new();

        for (key, value) in block_uv_config.tile_position {
            ...
            tile_position.insert(direction, [value.0, value.1]);
        }
        Ok(render::BlockUV {
