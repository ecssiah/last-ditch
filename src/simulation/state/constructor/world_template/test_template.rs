use crate::simulation::{constants::{SECTOR_RADIUS_IN_CELLS, WORLD_RADIUS_IN_SECTORS}, state::{
    World, world::{
        block,
        grid::{self},
    }
...
fn build_rooms(world: &mut World) {
    let world_radius_in_sectors = WORLD_RADIUS_IN_SECTORS as i32 - 1;
    ...
    for z in -world_radius_in_sectors..=world_radius_in_sectors {
        for y in -world_radius_in_sectors..=world_radius_in_sectors {
            for x in -world_radius_in_sectors..=world_radius_in_sectors {
                ...
                let sector_position =
                    grid::sector_coordinates_to_position(sector_coordinates);
...
fn build_central_room(world: &mut World) {
    ...
    let sector_position = grid::sector_coordinates_to_position(sector_coordinates);
