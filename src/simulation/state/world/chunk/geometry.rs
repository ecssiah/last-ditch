...
impl Geometry {
    ...
    pub fn vertices_and_indices(&self) -> (Vec<Point3<f32>>, Vec<[u32; 3]>) {
        ...
        for face in self.face_vec.iter() {
            let face_vertex_vec = face.vertices();
            let start_index = vertex_vec.len() as u32;

            for vertex in &face_vertex_vec {
                vertex_vec.push(Point3::new(vertex.x, vertex.y, vertex.z));
            }

            index_vec.push([start_index, start_index + 1, start_index + 2]);
            index_vec.push([start_index + 2, start_index + 3, start_index]);
        }
            ...
            while let Some(&(x, y, z)) = grid.iter().next() {
                ...
                match direction {
                    grid::Direction::XoYoZp | grid::Direction::XoYoZn => {
                        ...
                        vertex_vec.push(Point3::new(x0, y0, zf));
                        vertex_vec.push(Point3::new(x1, y0, zf));
                        vertex_vec.push(Point3::new(x1, y1, zf));
                        vertex_vec.push(Point3::new(x0, y1, zf));
                    ...
                    grid::Direction::XpYoZo | grid::Direction::XnYoZo => {
                        ...
                        vertex_vec.push(Point3::new(xf, y0, z0));
                        vertex_vec.push(Point3::new(xf, y1, z0));
                        vertex_vec.push(Point3::new(xf, y1, z1));
                        vertex_vec.push(Point3::new(xf, y0, z1));

                        index_vec.push([start_index, start_index + 1, start_index + 2]);
                        index_vec.push([start_index, start_index + 2, start_index + 3]);
                    }
                    grid::Direction::XoYpZo | grid::Direction::XoYnZo => {
                        ...
                        vertex_vec.push(Point3::new(x0, yf, z0));
                        vertex_vec.push(Point3::new(x1, yf, z0));
                        vertex_vec.push(Point3::new(x1, yf, z1));
                        vertex_vec.push(Point3::new(x0, yf, z1));
